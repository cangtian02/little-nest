...
class Articlelist extends React.Component {
  ...
  getArticleItem() {
    ...
    return (
      <Pull warper='home-content' forceUpdate={this.state.pageSize === this.state.maxPageSize - 1 ? 0 : 1} refresh={this.state.refresh} pullingDown={() => this.pullingDown()} pullingUp={() => this.pullingUp()}>
        {arr}
      </Pull>
    );
  }
  ...
  render() {
    return (
      <div className="home-item articleList" style={{ display: this.props.toggle ? 'block' : 'none' }}>
        {this.getArticleItem()}
      </div>
    );
  }
